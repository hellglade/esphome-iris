...
from esphome.components import remote_transmitter, remote_receiver, output
...
CONFIG_SCHEMA = cv.Schema({
    ...
    cv.Optional(CONF_CUSTOM_TRANSMITTER_PIN): output.pin,
})
...
async def to_code(config):
    ...
    # Optional custom GPIO pin for raw transmit
    if CONF_CUSTOM_TRANSMITTER_PIN in config:
        pin = await output.gpio_pin_expression(config[CONF_CUSTOM_TRANSMITTER_PIN])
