...
CONFIG_SCHEMA = cv.Schema(
    {
        ...
        cv.Optional(CONF_CUSTOM_TRANSMITTER_PIN): gpio.gpio_output_pin,
    }
)
...
async def to_code(config):
    ...
    # Set custom GPIO pin if defined
    if CONF_CUSTOM_TRANSMITTER_PIN in config:
        pin = await cg.gpio_pin_expression(config[CONF_CUSTOM_TRANSMITTER_PIN])
